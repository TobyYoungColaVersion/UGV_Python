...
class PSO(object):
    def __init__(self, num_task, data):
        self.num = 200  # 粒子数目
        self.num_task = num_task  # 任务数
        self.location = data  # 任务的位置坐标
        # 计算距离矩阵
        self.dis_mat = self.compute_dis_mat(num_task, self.location)  # 计算任务之间的距离矩阵
        # 初始化所有粒子
        self.particals = self.random_init(self.num, num_task)
    ...
    def greedy_init(self, dis_mat, num_total, num_task):
        start_index = 0
        result = []
        for i in range(num_total):
            rest = [x for x in range(0, num_task)]
            # 所有起始点都已经生成了
            if start_index >= num_task:
                start_index = np.random.randint(0, num_task)
    ...
    # 随机初始化
    def random_init(self, num_total, num_task):
        tmp = [x for x in range(num_task)]
        result = []
        for i in range(num_total):
            random.shuffle(tmp)
    ...
    # 计算不同任务之间的距离
    def compute_dis_mat(self, num_task, location):
        dis_mat = np.zeros((num_task, num_task))
    ...
    def cross(self, cur, best):
        one = cur.copy()
        l = [t for t in range(self.num_task)]
...
for d in range(1, num_drones):  # 1, ... drone-1
    to_process_idx.append(num_task - d)

model = PSO(num_task=data.shape[0], data=data.copy())
